...
T* vecs_read(const char* fname, size_t* d_out, size_t* n_out) {
    ...
    int d;
    (void)fread(&d, 1, sizeof(int), f); // Added (void) to suppress warnings
    fseek(f, 0, SEEK_SET);
    ...
    *n_out = n;
    ...
    for (size_t i = 0; i < n; i++) {
        (void)fread(header.data(), sizeof(int), 1, f); // Added (void)
        (void)fread(x + i * d, sizeof(T), d, f);       // Added (void)
    }
    fclose(f);
    return x;
}
...
void search_hnsw_conann_batch(
    ...
    std::vector<faiss::HNSWSearchCache>& all_caches) // Added faiss:: namespace here
{
    ef_used.resize(nq);
    ...
    for (size_t p = 0; p < params.ef_schedule.size(); ++p) {
        ...
        for (size_t i = 0; i < num_active; ++i) {
            ...
            // Map the active query to its dedicated cache
        ...
        index.search_resume(num_active, batch_queries.data(), k, 
...
int main(int argc, char* argv[]) {
    ...
    // -------------------------------------------------------------
    // STEP 2: Build the HNSW Index (Done once for both phases!)
    // -------------------------------------------------------------
    int M = 32;
    faiss::IndexHNSWFlat index(d_base, M);
    
    // Create an array of caches for the calibration queries
    std::vector<faiss::HNSWSearchCache> calib_caches;
    for (size_t i = 0; i < nq_calib; ++i) {
        calib_caches.emplace_back(1200);
    }

    std::cout << "2. Building HNSW index on " << nb << " vectors..." << std::endl;
    index.add(nb, xb);
    ...
    std::cout << "3. Running Calibration (Building Non-Conformity Matrix)..." << std::endl;
    for (size_t col = 0; col < num_ef; ++col) {
        int ef = efSearch_values[col];
        index.hnsw.efSearch = ef; 

        // Convert to pointer array for FAISS
        std::vector<faiss::HNSWSearchCache*> batch_calib_caches(nq_calib);
        for(size_t i = 0; i < nq_calib; i++) {
            batch_calib_caches[i] = &calib_caches[i];
        }
        index.search_resume(nq_calib, calib_queries, k, distances.data(), labels.data(), batch_calib_caches);
    ...
    // Store in-memory parameters
    ConannCalibParams params = {
        best_lambda, gamma, c_reg, global_min_dist, global_max_dist, efSearch_values
    };
    ...
    // Create an array of caches for the test queries
    ...
    search_hnsw_conann_batch(
        index, nq_test, test_queries, k, params, 
        all_labels.data(), all_distances.data(), tracking_ef_used, test_caches
    );
